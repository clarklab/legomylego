...
plate) whose inside clears the hub arms (r >= 9 studs). The knob axle passes through a
Dark Red Technic brick in the bottom ring."""
...
AXLE_CELLS = {(3, 8), (4, 8), (3, 9), (4, 9)}   # knob axle (x=80, y=18) crosses ring 2
...
def skirt(sub, disc_cells):
    ...
    for n, (kind, r_out, y) in enumerate(LAYERS):
        ...
        if n == 1:
            cells -= AXLE_CELLS
            sub.place("3700", "skirt_accent", (80, y, 170), tag="skirt_axle_brick")
        ...
        below = cells | ({(3, 8), (4, 8)} if n == 1 else set())
    return below
